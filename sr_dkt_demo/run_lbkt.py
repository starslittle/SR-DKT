...
from baselines.baseline_models import LBKT
...
def evaluate(model, loader, device):
    ...
    for batch in loader:
        ...
        if preds.shape[1] <= 1:
            continue
        valid = mask[:, 1:]
        y_true.extend(corrects[:, 1:][valid].detach().cpu().numpy().tolist())
...
def main():
    ...
    print(f"[LBKT] 使用设备: {device}")
    print(f"[LBKT] 数据集: ASSISTments 2017")
    ...
    model = LBKT(num_kc, CONFIG["hidden_size"]).to(device)
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        total_loss = 0.0
        steps = 0
        for batch in tqdm(train_loader, desc=f"[LBKT] Epoch {epoch}", leave=False):
            batch = move_batch(batch, device)
            optimizer.zero_grad()
            preds, _ = model(
        ...
        print(
            f"[LBKT] Epoch {epoch}/{CONFIG['epochs']} | "
        ...
        if val_auc > best_auc:
            ...
            if no_improve >= CONFIG["patience"]:
                print(f"[LBKT] Early stopping 触发，最佳 Val AUC: {best_auc:.3f}")
                break
    ...
    torch.save({"model_state_dict": model.state_dict(), "config": CONFIG}, CKPT_DIR / "LBKT.pt")

    test_metrics = evaluate(model, test_loader, device)
    print(f"[LBKT] 训练完成 | Test AUC: {test_metrics['AUC']:.3f} | ACC: {test_metrics['ACC']:.3f} | F1: {test_metrics['F1']:.3f}")
